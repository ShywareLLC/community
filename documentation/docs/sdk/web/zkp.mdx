...
The base protocol (preferred non-ZK tier) uses `SHA-256(voter_pub_key ‖ scoping_id)` as the identity hash. The IDV provider can correlate this with their own attestation record, making the receipt store linkable under lawful process.

The ZK tier replaces the identity hash with a **ZK nullifier**: `MiMC(person_secret, scoping_id)`. The IDV provider attests a `commitment = MiMC(person_secret)` as a private circuit witness — it never appears on-chain. Even a read-only IDV with full database access cannot map the on-chain nullifier back to a person's identity. Receipt store linkage is structurally impossible for any party, including the operator.
...
const nullifier = computeNullifier(personSecret, submissionId)
// → MiMC(personSecret, submissionId) hex — on-chain identity hash for this scoping period
```
...
```js
const { proofBytes, commitmentHex, nullifierHex } = await prover.generateProof(personSecret, submissionId)
// proofBytes: base64-encoded Groth16 proof
// The circuit proves:
//   MiMC(personSecret)            == commitmentHex   (Didit binding)
//   MiMC(personSecret, scopingId) == nullifierHex    (per-period dedup)
```
...
const envelope = await buildZKBallotEnvelope({
  ...
  choices: ['yes'],
  personSecret,
  diditCommitmentSigBase64: diditSig,   // Didit's Ed25519 sig over SHA-256(commitment ‖ scopingId)
  encSecret,                            // pass-through for receipt store
  timestamp: Math.floor(Date.now() / 1000)
})
// → {
//     txJson,          // TxTypeBallotCast payload — submit to /ballots
//     submissionId,    // H(submissionNonce) — direction-free public identifier
//     submissionNonce, // random 32-byte hex — private receipt; do not expose
//     nullifier,       // MiMC(personSecret, scopingId)
...
public inputs:     scoping_id, nullifier

constraint 1: MiMC(person_secret)             == commitment
constraint 2: MiMC(person_secret, scoping_id) == nullifier
```
