...
- Type-specific required fields are checked (e.g., `scoping_id`, ZK fields for ballots)
...
type BallotCastData struct {
    PollID             string
    ZKNullifier        string   // optional in ZK tier: MiMC(person_secret, scoping_id)
    ZKNullifierProof   []byte   // optional in ZK tier: Groth16 proof bytes
    ZKCommitment       string   // optional in ZK tier: MiMC(person_secret)
    DiditCommitmentSig []byte   // optional in ZK tier: sig over sha256(commitment ‖ scoping_id)
    ...
    BallotNonce        string   // random; submission_id = H(BallotNonce)
    Timestamp          int64
}
```

In the preferred non-ZK tier, the runtime derives `identity_hash` from
`SHA-256(voter_pub_key ‖ scoping_id)` after validating `voter_sig` and
...
type ConfirmReceiptData struct {
    PollID       string
    IdentityHash string  // ZK nullifier = MiMC(person_secret, scoping_id)
}
```
