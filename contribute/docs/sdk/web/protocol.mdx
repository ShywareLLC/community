...
:::warning
`H(uid)` as the identity commitment breaks cross-scoping unlinkability. This was a bug in an early ledger server implementation (fixed in commit [May 3 2026]). All new consumer code must use `deriveIdentityHash(uid, scopingId)`.
:::
...
// 4. Atomic delete via ledger adapter
await ledgerAdapter.rescindTwoListWrite(scopingId, submissionId, identityHash);
```

The current reference implementation uses session JWT for rescission (tests 17-18). This satisfies the structural two-list invariant but does not exercise the biometric re-derivation path. That path is planned as test 25.
...
const attestation = await signingAdapter.sign(`${l1Root}:${l2Root}:${count}`);

await ledgerAdapter.commitPeriodClose(scopingId, l1Root, l2Root, attestation);
```
...
:::note
Test 21 (planned) will explicitly verify that bulk-scan attempts against the reconcile interface return 404/405. The current suite tests per-participant access denial (tests 7, 15) but not a dedicated enumeration attempt.
:::
...
**Test 23 (planned)** will verify from the outside that no partial canonical state (`l1Count ≠ l2Count`) is ever observable between the start and completion of a `submitTwoListWrite` transaction.
