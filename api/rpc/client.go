// Package rpc provides a thin client for the CometBFT RPC and broadcast endpoints.
// The protocol API is a stateless proxy — no database, no auth, no session.
// Every query proxies directly to the CometBFT node.
package rpc
...
// Client wraps CometBFT RPC calls.
type Client struct {
	baseURL    string
...
// ABCIQuery queries the ABCI state machine at the given path.
